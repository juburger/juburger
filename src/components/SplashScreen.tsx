      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-cols-2 gap-2 mb-3">
            {categories.map(c => (
              <button key={c.id}
                className={`text-sm px-4 py-2.5 cursor-pointer rounded-full transition-all font-medium border-2 ${activeCat === c.id ? 'bg-foreground text-background border-foreground neu-sunken' : 'text-foreground border-border neu-flat'}`}
